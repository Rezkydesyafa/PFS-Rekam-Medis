<x-app-layout>
    <div class="mx-auto max-w-7xl flex flex-col gap-6">
        ...
        <div class="flex items-center gap-2 text-sm text-slate-500 dark:text-slate-400">
            ...
            <span class="material-symbols-outlined text-xs">chevron_right</span>
            <span class="font-medium text-slate-900 dark:text-white">Edit Obat</span>
        </div>

        <div class="flex flex-col gap-1">
            <h1 class="text-3xl font-bold text-slate-900 dark:text-white tracking-tight">Edit Data Obat</h1>
            <p class="text-slate-500 dark:text-slate-400">Perbarui informasi obat <span class="font-semibold text-slate-900 dark:text-white">{{ $obat->nama_obat }}</span>.</p>
        </div>

        <div class="rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-sm">
            <form method="POST" action="{{ route('obat.update', $obat->id_obat) }}" class="p-6 md:p-8">
                @csrf
                @method('PUT')
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="space-y-2">
                        ...
                        <input type="date" name="tanggal_kadaluarsa" id="tanggal_kadaluarsa" 
                               value="{{ old('tanggal_kadaluarsa', $obat->tanggal_kadaluarsa instanceof \Carbon\Carbon ? $obat->tanggal_kadaluarsa->format('Y-m-d') : $obat->tanggal_kadaluarsa) }}"
