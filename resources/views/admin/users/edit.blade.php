    ...
    <div class="max-w-3xl mx-auto space-y-6">
        ...
        <div class="flex flex-col gap-2">
            <div class="flex items-center gap-2 text-sm text-slate-500">
                ...
                <span class="text-primary font-medium">Edit User</span>
            </div>
            
            <h1 class="text-2xl font-bold text-slate-900 dark:text-white">Edit User</h1>
            <p class="text-slate-500 dark:text-slate-400">Perbarui informasi dan hak akses pengguna.</p>
        </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="space-y-2">
                        ...
                        <select name="role" id="role" required
                            ...
                            <option value="admin" {{ old('role', $user->role) == 'admin' ? 'selected' : '' }}>Administrator (Admin)</option>
                            <option value="unit_pendaftaran" {{ old('role', $user->role) == 'unit_pendaftaran' ? 'selected' : '' }}>Unit Pendaftaran</option>
                            <option value="petugas_rekam_medis" {{ old('role', $user->role) == 'petugas_rekam_medis' ? 'selected' : '' }}>Petugas Rekam Medis</option>
                            ...
                            <option value="petugas" {{ old('role', $user->role) == 'petugas' ? 'selected' : '' }}>Petugas (Legacy)</option>
                            <option value="petugas" {{ old('role', $user->role) == 'petugas' ? 'selected' : '' }}>Petugas (Legacy)</option>
                            <option value="petugas2" {{ old('role', $user->role) == 'petugas2' ? 'selected' : '' }}>Petugas 2 (Super Access)</option>
                        </select>
                        @error('role')
                            <p class="text-xs text-rose-500">{{ $message }}</p>
                        @enderror
                    ...
                    <div class="col-span-1 md:col-span-2 border-t border-slate-100 dark:border-slate-700 pt-4 mt-2">
                        <h3 class="text-sm font-bold text-slate-900 dark:text-white mb-4">Ganti Password (Opsional)</h3>
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <!-- Password -->
                            <div class="space-y-2">
                                ...
                                <input type="password" name="password" id="password"
                                    class="w-full px-3 py-2 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary text-sm">
                                <p class="text-[10px] text-slate-500">Kosongkan jika tidak ingin mengganti password.</p>
                                @error('password')
                                    <p class="text-xs text-rose-500">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <!-- Confirm Password -->
                            <div class="space-y-2">
                                <label for="password_confirmation" class="text-sm font-medium text-slate-700 dark:text-slate-300">Konfirmasi Password Baru</label>
